...
for (i in 1:5000){
  ...
  source('./Bodies/bodyPEAR_poisson.R')
...
lapply(primaryset, cat, '\n', file = 'psPEAR_pois.txt', append = T)
## Write big matrix into a file
write.csv(sumup, 'PEAR_pois.csv')
...
rownames(newmatrix) <- 'PEARpois'
colnames(newmatrix) <- c('ptA', 'ptB', 'ptC', 'ptD', 'ptE',
                         'AB', 'AB+al.', 'A+al.', 'B+al.', 'Other', 'Empty', 'Full',
                         'Single', 'Time')

rownames(percent) <- 'PEARpois'
colnames(percent) <- c('ptA', 'ptB', 'ptC', 'ptD', 'ptE',
                       'AB', 'AB+al.', 'A+al.', 'B+al.', 'Other', 'Empty', 'Full',
                       'Single', 'Time')

write.csv(newmatrix, 'collapsedPEARpois_counts.csv')
write.csv(percent, 'collapsedPEARpois_percent.csv')
