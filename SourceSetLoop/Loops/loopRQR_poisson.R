## RANDOMIZED QUANTILE RESIDUALS (POISSON) DATA ####
...
for (i in 1:5000){
  ...
  source('./Bodies/bodyRQR_poisson.R')
...
lapply(primaryset, cat, '\n', file = 'psRQR_pois.txt', append = T)
## Write big matrix into a file
write.csv(sumup, 'RQR_pois.csv')
...
rownames(newmatrix) <- 'RQRpois'
colnames(newmatrix) <- c('ptA', 'ptB', 'ptC', 'ptD', 'ptE',
                         'AB', 'AB+al.', 'A+al.', 'B+al.', 'Other', 'Empty', 'Full',
                         'Single', 'Time')

rownames(percent) <- 'RQRpois'
colnames(percent) <- c('ptA', 'ptB', 'ptC', 'ptD', 'ptE',
                       'AB', 'AB+al.', 'A+al.', 'B+al.', 'Other', 'Empty', 'Full',
                       'Single', 'Time')

write.csv(newmatrix, 'collapsedRQRpois_counts.csv')
write.csv(percent, 'collapsedRQRpois_percent.csv')
