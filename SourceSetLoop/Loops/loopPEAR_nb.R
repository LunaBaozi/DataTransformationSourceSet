...
lapply(primaryset, cat, '\n', file = 'psPEAR_nb.txt', append = T)
## Write big matrix into a file
write.csv(sumup, 'PEAR_nb.csv')
...
rownames(newmatrix) <- 'PEARnb'
colnames(newmatrix) <- c('ptA', 'ptB', 'ptC', 'ptD', 'ptE',
                         'AB', 'AB+al.', 'A+al.', 'B+al.', 'Other', 'Empty', 'Full',
                         'Single', 'Time')

rownames(percent) <- 'PEARnb'
colnames(percent) <- c('ptA', 'ptB', 'ptC', 'ptD', 'ptE',
                       'AB', 'AB+al.', 'A+al.', 'B+al.', 'Other', 'Empty', 'Full',
                       'Single', 'Time')

write.csv(newmatrix, 'collapsedPEARnb_counts.csv')
write.csv(percent, 'collapsedPEARnb_percent.csv')
