...
lapply(primaryset, cat, '\n', file = 'psDEV_nb.txt', append = T)
## Write big matrix into a file
write.csv(sumup, 'DEV_nb.csv')
...
rownames(newmatrix) <- 'DEVnb'
colnames(newmatrix) <- c('ptA', 'ptB', 'ptC', 'ptD', 'ptE',
                         'AB', 'AB+al.', 'A+al.', 'B+al.', 'Other', 'Empty', 'Full',
                         'Single', 'Time')

rownames(percent) <- 'DEVnb'
colnames(percent) <- c('ptA', 'ptB', 'ptC', 'ptD', 'ptE',
                       'AB', 'AB+al.', 'A+al.', 'B+al.', 'Other', 'Empty', 'Full',
                       'Single', 'Time')

write.csv(newmatrix, 'collapsedDEVnb_counts.csv')
write.csv(percent, 'collapsedDEVnb_percent.csv')
