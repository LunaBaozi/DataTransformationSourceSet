...
for (i in 1:5000){
  ...
  source('./Bodies/bodyDEV_poisson.R')
...
lapply(primaryset, cat, '\n', file = 'psDEV_pois.txt', append = T)
## Write big matrix into a file
write.csv(sumup, 'DEV_pois.csv')
...
rownames(newmatrix) <- 'DEVpois'
colnames(newmatrix) <- c('ptA', 'ptB', 'ptC', 'ptD', 'ptE',
                         'AB', 'AB+al.', 'A+al.', 'B+al.', 'Other', 'Empty', 'Full',
                         'Single', 'Time')

rownames(percent) <- 'DEVpois'
colnames(percent) <- c('ptA', 'ptB', 'ptC', 'ptD', 'ptE',
                       'AB', 'AB+al.', 'A+al.', 'B+al.', 'Other', 'Empty', 'Full',
                       'Single', 'Time')

write.csv(newmatrix, 'collapsedDEVpois_counts.csv')
write.csv(percent, 'collapsedDEVpois_percent.csv')
